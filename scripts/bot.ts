...
const bot = async (): Promise<void> => {
    ...
    for (;;) {
        ...
        if (
            ...
            // Discord Webhook
            sendWithdrawLogsWebhook(data, poolConfig);
            // Google Sheets API
            sendWithdrawLogsGSheet(doc, data, token0, token1);
    ...
    // Discord Webhook
    sendMintLogsWebhook(mintLogsParams);
    // Google Sheets API
    sendMintLogsGSheet(doc, mintLogsParams, token0, token1);
