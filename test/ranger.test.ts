...
describe("Ranger", async () => {
	...
	it("Mint new position", async () => {
		...
		const nfmp: INonfungiblePositionManager =
	...
	it("Collect all fees and withdraw position", async () => {
		...
		const token0_before_balance =
			await token0.balanceOf(contractAddress);
		const token1_before_balance =
		...
		const token0_after_balance =
			await token0.balanceOf(contractAddress);
		const token1_after_balance =
			await token1.balanceOf(contractAddress);
		...
		expect(token0_after_balance).to.greaterThan(
			token0_before_balance,
		);
		expect(token1_after_balance).to.greaterThan(
			token1_before_balance,
		);
	});
	...
	it("Withdraw funds from contract to owner", async () => {
		...
		await expect(notowner.safeWithdraw()).to.be.revertedWith(
			"Not Owner!",
		);
		...
		expect(token1_deployer_after_balance).to.equal(
			token1_deployer_before_balance +
				token1_contract_before_balance,
		);
	});
});
